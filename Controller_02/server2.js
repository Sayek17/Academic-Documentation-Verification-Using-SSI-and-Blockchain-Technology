...
app.post("/newCon", async function (req, res) {
  ...
  axios
    .post(
      "http://127.0.0.1:8041/connections/create-invitation?alias=" + memoName,
      bodyData
    )
    .then((resp) => {
      if (resp) {
...
app.post("/checkMemName", function (req, res) {
  ...
  axios
    .get("http://127.0.0.1:8041/connections")
    .then((resp) => {
      let flag = false;
      let conID;
...
app.post("/offerCredential", async function (req, res) {
  ...
  axios
    .get("http://127.0.0.1:8041/credential-definitions/created")
    .then((resp) => {
      ...
      if (credID) {
        ...
        axios.post("http://127.0.0.1:8041/issue-credential/send-offer", data);
  ...
  axios
    ...
    .then((resp) => {
      ...
      if (req.cookies.conID) {
        ...
        axios
          .post("http://127.0.0.1:8041/present-proof/send-request", data)
          .then((resp) => {
...
//logout
app.get("/logoutEndPoint", async (req, res) => {
  res.setHeader("Access-Control-Allow-Origin", "*");
  req.session.destroy();
  global.connectionId = null;
  global.connectionStatus = null;
...
app.post("/twoFactorCheck", function (req, res) {
  ...
  if (req.cookies.conID && req.cookies.memoName) {
    ...
    TwoFactorDataSchema.remove(
      { connectionID: connectionID, memoName: memoName, code: codeData },
      (err) => {
        if (!err) {
          res.render("connected");
        } else {
...
app.post("/receive-invitation", async function (req, res) {
  console.log("At Post receive invitation!");

  const inviationPayload = req.body.invData;
  console.log("server.js ln:924");
  const parsedData = JSON.parse(inviationPayload);
  console.log(parsedData);

  axios
    .post("http://127.0.0.1:8041/connections/receive-invitation", parsedData)
    .then((resp) => {
      console.log(`Existing Cred Resp: ${resp}`);
      res.json({ status: 200 });
    })
    .catch((err) => {
      console.error(err);
 ...
 *************************************************************/
const configPath = path.join(__dirname, "../common-config.json"); // Adjust the path based on your directory structure
const rawConfig = fs.readFileSync(configPath, "utf8");
const config = JSON.parse(rawConfig);
const localHostAddress = config.localhost;
console.log(`Dekho: ${localHostAddress}`);

// app.listen(8888, localHostAddress, () => {
